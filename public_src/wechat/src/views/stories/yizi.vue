<template lang="html">
  <div class="">
    <div class="story-top">
      ...
      <audio @ended="isPlayingStory=false" :src="story.fileName" id="audio">
      </audio>
    </div>
    <div class="b_add_btn" @click="popup(0)">
      ...
      <div class="audio-block" v-show="showAudioBlock0">
        <img @click="playVoice(1)" src="../../assets/audio_btn.png" alt="" />
    ...
    <div class="a_add_btn" @click="popup(2)">
        ...
        <div class="audio-block" v-show="showAudioBlock1">
          <img @click="playVoice(3)" src="../../assets/audio_btn.png" alt="" />
          <i class="rerecord" @click="popup(3)"></i>
          <i class="delete" @click="deleteRecord(3)"></i>
        </div>
    </div>
    <div class="push_btn fix_bottom">
      <div class="push_story_btn bdr" @click="playStory()">
        <div><img src="../../assets/icon_nav_button.png" alt="" /></div>
        <p>{{isPlayingStory?'暂停':'试听'}}</p>
      </div>
      <div class="push_story_btn" @click="pushStory()">
        <div><img src="../../assets/icon_nav_button.png" alt="" /></div>
        <p>推送</p>
      </div>
...
export default {
  ...
  computed: {
    showAudioBlock0 () {
      return (this.voices[0].localId | this.voices[1].localId)
    },
    showAudioBlock1 () {
      return (this.voices[2].localId | this.voices[3].localId)
    }
  },
  ready: function () {
    this.$http.get(this.$http.options.root + '/wx/jsapi?url=' + window.location.href.replace('localhost:8080', 'czcioutest.ittun.com')).then(function (res) {
  ...
  methods: {
    ...
    stopRecord () {
      this.isRecording = false
      wx.stopVoice({
        localId: this.voices[this.recordType].localId
      })
    },
    uploadVoice () {
      // 上传音频
      if (this.voices[1].localId) {
        this.wechatVoiceUpload(1)
      } else if (this.voices[0].localId) {
        this.wechatVoiceUpload(0)
      }
      if (this.voices[3].localId) {
        this.wechatVoiceUpload(3)
      } else if (this.voices[2].localId) {
        this.wechatVoiceUpload(2)
      }
    },
    wechatVoiceUpload (i) {
      wx.uploadVoice({
        ...
        success: function (res) {
          this.voices[i].serverId = res.serverId
        },
        fail: function (res) {
        }
      })
    ...
    playVoice (i) {
      ...
      if (!this.isPlaying) {
        this.isPlaying = true
        wx.playVoice({
          localId: this.voices[playRecordIndex].localId
        })
      } else {
...
<style lang="css">
    ...
    .a_add_btn , .b_add_btn{height:60px;width:100%;padding-top:30px;padding-bottom:30px;border-bottom:1px solid #d3d5dc;background-color:#fff;}
	  div.ok{opacity:1;}
    .a_add_btn div, .b_add_btn div{display:inline-block;float:left;}
    .a_add_btn div.image , .b_add_btn div.image{width:100px;height:60px;}
    ...
    .fix_bottom{position:fixed;bottom:50px;}
    ...
    .rerecord{background-image: url(../../assets/re_record.png);right: 55px;}
    .delete{ackground-image: url(../../assets/del_record.png);right: 5px;}
